...
export class TipAllocationsService {
  ...
  async create(
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to create tip allocations',
      );
    }

    const tip = await this.tipRepository.findOne({
      where: {
        id: createTipAllocationDto.tipId,
    ...
    if (tip.record_status === TipRecordStatus.DELETED) {
      throw new BadRequestException(
        'Cannot create allocation for a deleted tip',
      );
    }

    const collaborator = await this.collaboratorRepository.findOne({
      where: {
        id: createTipAllocationDto.collaboratorId,
        merchant_id: authenticatedUserMerchantId,
      },
    });
    if (!collaborator) {
    ...
    const shift = await this.shiftRepository.findOne({
      where: {
        id: createTipAllocationDto.shiftId,
  ...
  async findAll(
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to access tip allocations',
      );
    }
    ...
    const qb = this.tipAllocationRepository
      ...
      .andWhere('tip.record_status != :tipDeleted', {
        tipDeleted: TipRecordStatus.DELETED,
      })
    ...
    if (query.createdDate) {
      ...
      qb.andWhere('allocation.created_at >= :createdStart', {
        createdStart: startDate,
      });
      qb.andWhere('allocation.created_at < :createdEnd', {
        createdEnd: endDate,
      });
    }
  ...
  async findOne(
    ...
    if (!id || id <= 0) {
      throw new BadRequestException(
        'Tip allocation ID must be a valid positive number',
      );
    }
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
    ...
    const allocation = await this.tipAllocationRepository
      ...
      .andWhere('tip.merchant_id = :merchantId', {
        merchantId: authenticatedUserMerchantId,
      })
      .andWhere('allocation.record_status != :deletedStatus', {
        deletedStatus: TipAllocationRecordStatus.DELETED,
  ...
  async update(
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to update tip allocations',
      );
    }
    ...
    if (
      ...
      (updateTipAllocationDto.percentage < 0 ||
        updateTipAllocationDto.percentage > 100)
    ) {
      throw new BadRequestException('Percentage must be between 0 and 100');
    }
    if (
      updateTipAllocationDto.amount !== undefined &&
      updateTipAllocationDto.amount < 0
    ) {
      throw new BadRequestException(
        'Amount must be greater than or equal to 0',
      );
    ...
    if (updateTipAllocationDto.tipId !== undefined) {
      const tip = await this.tipRepository.findOne({
        where: {
          id: updateTipAllocationDto.tipId,
          merchant_id: authenticatedUserMerchantId,
        },
      });
      if (!tip) {
    ...
    if (updateTipAllocationDto.shiftId !== undefined) {
      const shift = await this.shiftRepository.findOne({
        where: {
          id: updateTipAllocationDto.shiftId,
          merchantId: authenticatedUserMerchantId,
        },
      });
      if (!shift) {
    ...
    if (updateTipAllocationDto.tipId !== undefined)
      updateData.tip_id = updateTipAllocationDto.tipId;
    if (updateTipAllocationDto.collaboratorId !== undefined)
      updateData.collaborator_id = updateTipAllocationDto.collaboratorId;
    if (updateTipAllocationDto.shiftId !== undefined)
      updateData.shift_id = updateTipAllocationDto.shiftId;
    if (updateTipAllocationDto.role !== undefined)
      updateData.role = updateTipAllocationDto.role;
    if (updateTipAllocationDto.percentage !== undefined)
      updateData.percentage = updateTipAllocationDto.percentage;
    if (updateTipAllocationDto.amount !== undefined)
      updateData.amount = updateTipAllocationDto.amount;
  ...
  async remove(
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to delete tip allocations',
      );
    }
