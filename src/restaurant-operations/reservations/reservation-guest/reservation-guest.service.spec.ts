...
describe('ReservationGuestService', () => {
  ...
  describe('create', () => {
    ...
    it('should fail if reservation belongs to another merchant', async () => {
      ...
      await expect(service.create(createDto, merchantId)).rejects.toThrow('Reservation not found or does not belong to your merchant');
    });
  ...
  describe('findAllGlobal', () => {
    ...
    it('should apply ILike filter when name is provided', async () => {
      ...
      expect(mockGuestRepository.findAndCount).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('findOne', () => {
    it('should return a guest by ID if merchant matches', async () => {
      mockGuestRepository.findOne.mockResolvedValue({ id: 1, name: 'John', reservation: { merchant_id: 1 } });
      const result = await service.findOne(1, 1);
      expect(result.data.id).toBe(1);
    });

    it('should throw if guest belongs to another merchant', async () => {
      mockGuestRepository.findOne.mockResolvedValue({ id: 1, name: 'John', reservation: { merchant_id: 2 } });
  ...
  describe('update', () => {
    it('should update guest info successfully', async () => {
      ...
      const result = await service.update(1, { name: 'New', email: 'new@test.com' }, 1);
      expect(result.data.name).toBe('New');
      expect(mockGuestRepository.save).toHaveBeenCalled();
    });

    it('should fail if guest belongs to another merchant', async () => {
      mockGuestRepository.findOne.mockResolvedValue({ id: 1, reservation: { merchant_id: 2 } });
      await expect(service.update(1, { name: 'New' }, 1)).rejects.toThrow('Guest not found');
    });
  });
