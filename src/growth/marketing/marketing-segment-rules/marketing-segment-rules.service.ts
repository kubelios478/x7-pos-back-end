...
import {
  GetMarketingSegmentRuleQueryDto,
  MarketingSegmentRuleSortBy,
} from './dto/get-marketing-segment-rule-query.dto';
import {
  MarketingSegmentRuleResponseDto,
...
export class MarketingSegmentRulesService {
  ...
  async create(
    createMarketingSegmentRuleDto: CreateMarketingSegmentRuleDto,
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to create marketing segment rules',
      );
    }
    ...
    const segment = await this.marketingSegmentRepository
      ...
      .where('segment.id = :segmentId', {
        segmentId: createMarketingSegmentRuleDto.segmentId,
      })
      .andWhere('segment.merchant_id = :merchantId', {
        merchantId: authenticatedUserMerchantId,
      })
      .andWhere('segment.status != :deletedStatus', {
    ...
    if (!segment) {
      throw new NotFoundException(
        'Marketing segment not found or you do not have access to it',
      );
    }

    // Business rule validation: field must not be empty
    if (
      !createMarketingSegmentRuleDto.field ||
      createMarketingSegmentRuleDto.field.trim().length === 0
    ) {
    ...
    if (
      !createMarketingSegmentRuleDto.value ||
      createMarketingSegmentRuleDto.value.trim().length === 0
    ) {
    ...
    const savedMarketingSegmentRule =
      await this.marketingSegmentRuleRepository.save(marketingSegmentRule);

    // Fetch the complete marketing segment rule with relations
    const completeMarketingSegmentRule =
      await this.marketingSegmentRuleRepository.findOne({
    ...
    if (!completeMarketingSegmentRule) {
      throw new NotFoundException(
        'Marketing segment rule not found after creation',
      );
    }
    ...
    return {
      ...
      data: this.formatMarketingSegmentRuleResponse(
        completeMarketingSegmentRule,
      ),
    };
  }

  async findAll(
    query: GetMarketingSegmentRuleQueryDto,
    authenticatedUserMerchantId: number | null | undefined,
  ): Promise<PaginatedMarketingSegmentRuleResponseDto> {
    // Validate user has merchant
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to access marketing segment rules',
      );
    }
    ...
    queryBuilder.andWhere('rule.status != :deletedStatus', {
      deletedStatus: MarketingSegmentRuleStatus.DELETED,
    });

    if (query.segmentId) {
      queryBuilder.andWhere('rule.segment_id = :segmentId', {
        segmentId: query.segmentId,
      });
    }

    if (query.field) {
      queryBuilder.andWhere('rule.field ILIKE :field', {
        field: `%${query.field}%`,
      });
    }

    if (query.operator) {
      queryBuilder.andWhere('rule.operator = :operator', {
    ...
    if (query.sortBy) {
      const sortField =
        query.sortBy === MarketingSegmentRuleSortBy.FIELD
          ? 'rule.field'
          : query.sortBy === MarketingSegmentRuleSortBy.OPERATOR
            ? 'rule.operator'
            : query.sortBy === MarketingSegmentRuleSortBy.CREATED_AT
              ? 'rule.created_at'
              : query.sortBy === MarketingSegmentRuleSortBy.UPDATED_AT
                ? 'rule.updated_at'
                : 'rule.id';
    ...
    return {
      ...
      data: marketingSegmentRules.map((rule) =>
        this.formatMarketingSegmentRuleResponse(rule),
      ),
      paginationMeta,
    };
  }
  ...
  async findOne(
    ...
    if (!id || id <= 0) {
      throw new BadRequestException(
        'Marketing segment rule ID must be a valid positive number',
      );
    }
    ...
    const marketingSegmentRule = await this.marketingSegmentRuleRepository
      ...
      .andWhere('rule.status != :deletedStatus', {
        deletedStatus: MarketingSegmentRuleStatus.DELETED,
      })
      .getOne();
  ...
  async update(
    id: number,
    updateMarketingSegmentRuleDto: UpdateMarketingSegmentRuleDto,
    authenticatedUserMerchantId: number | null | undefined,
  ): Promise<OneMarketingSegmentRuleResponseDto> {
    // Validate ID
    if (!id || id <= 0) {
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to update marketing segment rules',
      );
    }

    // Find existing marketing segment rule
    const existingMarketingSegmentRule =
      await this.marketingSegmentRuleRepository
        .createQueryBuilder('rule')
        .leftJoinAndSelect('rule.segment', 'segment')
        .leftJoinAndSelect('segment.merchant', 'merchant')
        .where('rule.id = :id', { id })
    ...
    if (updateMarketingSegmentRuleDto.segmentId !== undefined) {
      const segment = await this.marketingSegmentRepository
        .createQueryBuilder('segment')
        .where('segment.id = :segmentId', {
          segmentId: updateMarketingSegmentRuleDto.segmentId,
        })
        .andWhere('segment.merchant_id = :merchantId', {
          merchantId: authenticatedUserMerchantId,
        })
    ...
    if (updateMarketingSegmentRuleDto.field !== undefined) {
      if (
        !updateMarketingSegmentRuleDto.field ||
        updateMarketingSegmentRuleDto.field.trim().length === 0
      ) {
        throw new BadRequestException('Field cannot be empty');
      }
    ...
    if (updateMarketingSegmentRuleDto.value !== undefined) {
      if (
        !updateMarketingSegmentRuleDto.value ||
        updateMarketingSegmentRuleDto.value.trim().length === 0
      ) {
        throw new BadRequestException('Value cannot be empty');
      }
    ...
    if (updateMarketingSegmentRuleDto.segmentId !== undefined)
      updateData.segment_id = updateMarketingSegmentRuleDto.segmentId;
    if (updateMarketingSegmentRuleDto.field !== undefined)
      updateData.field = updateMarketingSegmentRuleDto.field.trim();
    if (updateMarketingSegmentRuleDto.operator !== undefined)
      updateData.operator = updateMarketingSegmentRuleDto.operator;
    if (updateMarketingSegmentRuleDto.value !== undefined)
      updateData.value = updateMarketingSegmentRuleDto.value.trim();

    await this.marketingSegmentRuleRepository.update(id, updateData);

    // Fetch updated marketing segment rule
    const updatedMarketingSegmentRule =
      await this.marketingSegmentRuleRepository.findOne({
        where: { id },
        relations: ['segment', 'segment.merchant'],
      });
    ...
    if (!updatedMarketingSegmentRule) {
      throw new NotFoundException(
        'Marketing segment rule not found after update',
      );
    }
  ...
  async remove(
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to delete marketing segment rules',
      );
    }
    ...
    existingMarketingSegmentRule.status = MarketingSegmentRuleStatus.DELETED;
    await this.marketingSegmentRuleRepository.save(
      existingMarketingSegmentRule,
    );
  ...
  private formatMarketingSegmentRuleResponse(
    marketingSegmentRule: MarketingSegmentRule,
  ): MarketingSegmentRuleResponseDto {
    return {
