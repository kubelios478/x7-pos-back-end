...
import {
  GetMarketingAutomationActionQueryDto,
  MarketingAutomationActionSortBy,
} from './dto/get-marketing-automation-action-query.dto';
import {
  MarketingAutomationActionResponseDto,
...
export class MarketingAutomationActionsService {
  ...
  async create(
    createMarketingAutomationActionDto: CreateMarketingAutomationActionDto,
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to create marketing automation actions',
      );
    }
    ...
    const automation = await this.marketingAutomationRepository
      ...
      .where('automation.id = :automationId', {
        automationId: createMarketingAutomationActionDto.automationId,
      })
    ...
    marketingAutomationAction.automation_id =
      createMarketingAutomationActionDto.automationId;
    marketingAutomationAction.sequence =
      createMarketingAutomationActionDto.sequence;
    marketingAutomationAction.action_type =
      createMarketingAutomationActionDto.actionType;
    marketingAutomationAction.target_id =
      createMarketingAutomationActionDto.targetId ?? null;
    marketingAutomationAction.payload =
      createMarketingAutomationActionDto.payload ?? null;
    marketingAutomationAction.delay_seconds =
      createMarketingAutomationActionDto.delaySeconds ?? null;
    marketingAutomationAction.status = MarketingAutomationActionStatus.ACTIVE;

    const savedMarketingAutomationAction =
      await this.marketingAutomationActionRepository.save(
        marketingAutomationAction,
      );

    // Fetch the complete marketing automation action with relations
    const completeMarketingAutomationAction =
      await this.marketingAutomationActionRepository.findOne({
    ...
    if (!completeMarketingAutomationAction) {
      throw new NotFoundException(
        'Marketing automation action not found after creation',
      );
    }
    ...
    return {
      ...
      data: this.formatMarketingAutomationActionResponse(
        completeMarketingAutomationAction,
      ),
    };
  }

  async findAll(
    query: GetMarketingAutomationActionQueryDto,
    authenticatedUserMerchantId: number | null | undefined,
  ): Promise<PaginatedMarketingAutomationActionResponseDto> {
    // Validate user has merchant
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to access marketing automation actions',
      );
    }
    ...
    queryBuilder.andWhere('action.status != :deletedStatus', {
      deletedStatus: MarketingAutomationActionStatus.DELETED,
    });

    if (query.automationId) {
      queryBuilder.andWhere('action.automation_id = :automationId', {
        automationId: query.automationId,
      });
    }

    if (query.actionType) {
      queryBuilder.andWhere('action.action_type = :actionType', {
        actionType: query.actionType,
      });
    }

    if (query.targetId) {
      queryBuilder.andWhere('action.target_id = :targetId', {
    ...
    if (query.sortBy) {
      const sortField =
        query.sortBy === MarketingAutomationActionSortBy.SEQUENCE
          ? 'action.sequence'
          : query.sortBy === MarketingAutomationActionSortBy.ACTION_TYPE
            ? 'action.action_type'
            : query.sortBy === MarketingAutomationActionSortBy.CREATED_AT
              ? 'action.created_at'
              : 'action.id';
    ...
    const [marketingAutomationActions, total] =
    ...
    return {
      ...
      data: marketingAutomationActions.map((action) =>
        this.formatMarketingAutomationActionResponse(action),
      ),
      paginationMeta,
    };
  }
  ...
  async findOne(
    ...
    if (!id || id <= 0) {
      throw new BadRequestException(
        'Marketing automation action ID must be a valid positive number',
      );
    }
    ...
    const marketingAutomationAction =
      await this.marketingAutomationActionRepository
        .createQueryBuilder('action')
        .leftJoinAndSelect('action.automation', 'automation')
        .leftJoin('automation.merchant', 'merchant')
        ...
        .andWhere('action.status != :deletedStatus', {
          deletedStatus: MarketingAutomationActionStatus.DELETED,
        })
        .getOne();
  ...
  async update(
    id: number,
    updateMarketingAutomationActionDto: UpdateMarketingAutomationActionDto,
    authenticatedUserMerchantId: number | null | undefined,
  ): Promise<OneMarketingAutomationActionResponseDto> {
    // Validate ID
    if (!id || id <= 0) {
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to update marketing automation actions',
      );
    }

    // Find existing marketing automation action
    const existingMarketingAutomationAction =
      await this.marketingAutomationActionRepository
        .createQueryBuilder('action')
        .leftJoinAndSelect('action.automation', 'automation')
        .leftJoin('automation.merchant', 'merchant')
    ...
    if (updateMarketingAutomationActionDto.automationId !== undefined) {
      const automation = await this.marketingAutomationRepository
        .createQueryBuilder('automation')
        .where('automation.id = :automationId', {
          automationId: updateMarketingAutomationActionDto.automationId,
        })
        .andWhere('automation.merchant_id = :merchantId', {
          merchantId: authenticatedUserMerchantId,
        })
    ...
    if (
      updateMarketingAutomationActionDto.sequence !== undefined &&
      updateMarketingAutomationActionDto.sequence < 1
    ) {
      throw new BadRequestException('Sequence must be greater than 0');
    }
    ...
    // Validate payload is valid JSON if provided
    if (
      updateMarketingAutomationActionDto.payload !== undefined &&
      updateMarketingAutomationActionDto.payload !== null
    ) {
      try {
    ...
    if (updateMarketingAutomationActionDto.automationId !== undefined)
      updateData.automation_id =
        updateMarketingAutomationActionDto.automationId;
    if (updateMarketingAutomationActionDto.sequence !== undefined)
      updateData.sequence = updateMarketingAutomationActionDto.sequence;
    if (updateMarketingAutomationActionDto.actionType !== undefined)
      updateData.action_type = updateMarketingAutomationActionDto.actionType;
    if (updateMarketingAutomationActionDto.targetId !== undefined)
      updateData.target_id = updateMarketingAutomationActionDto.targetId;
    if (updateMarketingAutomationActionDto.payload !== undefined)
      updateData.payload = updateMarketingAutomationActionDto.payload || null;
    if (updateMarketingAutomationActionDto.delaySeconds !== undefined)
      updateData.delay_seconds =
        updateMarketingAutomationActionDto.delaySeconds;

    await this.marketingAutomationActionRepository.update(id, updateData);

    // Fetch updated marketing automation action
    const updatedMarketingAutomationAction =
      await this.marketingAutomationActionRepository.findOne({
        where: { id },
        relations: ['automation'],
      });
    ...
    if (!updatedMarketingAutomationAction) {
      throw new NotFoundException(
        'Marketing automation action not found after update',
      );
    }
    ...
    return {
      ...
      data: this.formatMarketingAutomationActionResponse(
        updatedMarketingAutomationAction,
      ),
    };
  }
  ...
  async remove(
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to delete marketing automation actions',
      );
    }
    ...
    if (
      existingMarketingAutomationAction.status ===
      MarketingAutomationActionStatus.DELETED
    ) {
      throw new ConflictException(
        'Marketing automation action is already deleted',
      );
    }
    ...
    existingMarketingAutomationAction.status =
      MarketingAutomationActionStatus.DELETED;
    await this.marketingAutomationActionRepository.save(
      existingMarketingAutomationAction,
    );
  ...
  private formatMarketingAutomationActionResponse(
    marketingAutomationAction: MarketingAutomationAction,
  ): MarketingAutomationActionResponseDto {
    return {
