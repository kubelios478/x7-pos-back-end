...
describe('MarketingAutomationActionsService', () => {
  ...
  let marketingAutomationActionRepository: jest.Mocked<
    Repository<MarketingAutomationAction>
  >;
  let marketingAutomationRepository: jest.Mocked<
    Repository<MarketingAutomation>
  >;
  ...
  const mockQueryBuilder = {
    ...
    getManyAndCount: jest
      .fn()
      .mockResolvedValue([[mockMarketingAutomationAction], 1]),
  };
  ...
  beforeEach(async () => {
    ...
    marketingAutomationActionRepository = module.get(
      getRepositoryToken(MarketingAutomationAction),
    );
    marketingAutomationRepository = module.get(
  ...
  describe('create', () => {
    it('should create a marketing automation action successfully', async () => {
      ...
      marketingAutomationRepository.createQueryBuilder().getOne = jest
        .fn()
        .mockResolvedValue(mockAutomation);
      marketingAutomationActionRepository.save = jest
        .fn()
        .mockResolvedValue(mockMarketingAutomationAction);
      marketingAutomationActionRepository.findOne = jest
        .fn()
        .mockResolvedValue(mockMarketingAutomationAction);
      ...
      expect(result.data.actionType).toBe(
        MarketingAutomationActionType.SEND_EMAIL,
      );
    });
    ...
    it('should throw ForbiddenException when user is not associated with a merchant', async () => {
      ...
      await expect(service.create(createDto, undefined)).rejects.toThrow(
        ForbiddenException,
      );
    });
  ...
  describe('findAll', () => {
    it('should return paginated marketing automation actions', async () => {
      ...
      mockQueryBuilder.getManyAndCount = jest
        .fn()
        .mockResolvedValue([[mockMarketingAutomationAction], 1]);
      marketingAutomationActionRepository.createQueryBuilder = jest.fn(
      ...
      expect(result.message).toBe(
        'Marketing automation actions retrieved successfully',
      );
    ...
    it('should throw ForbiddenException when user is not associated with a merchant', async () => {
      ...
      await expect(service.findAll(query, null)).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('should filter by automationId', async () => {
      ...
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith(
        'action.automation_id = :automationId',
        { automationId: 1 },
      );
      expect(result.data).toHaveLength(1);
    });
  ...
  describe('findOne', () => {
    it('should return a marketing automation action by ID', async () => {
      ...
      expect(result.statusCode).toBe(200);
      expect(result.message).toBe(
        'Marketing automation action retrieved successfully',
      );
    ...
    it('should throw ForbiddenException when user is not associated with a merchant', async () => {
      await expect(service.findOne(1, null)).rejects.toThrow(
        ForbiddenException,
      );
    });
  ...
  describe('update', () => {
    it('should update a marketing automation action successfully', async () => {
      ...
      marketingAutomationActionRepository.update = jest
        .fn()
      ...
      expect(result.statusCode).toBe(200);
      expect(result.message).toBe(
        'Marketing automation action updated successfully',
      );
    ...
    it('should validate new automation when automationId is updated', async () => {
      ...
      marketingAutomationRepository.createQueryBuilder = jest.fn(
        () => mockQueryBuilder as any,
      );
